            ...
            while True:
                with ThreadPoolExecutor() as executor:
                    futures = [
                        ...
                        executor.submit(send_sms.RahmiGokhan),
                        executor.submit(send_sms.RenkliSeyler),
                        executor.submit(send_sms.Sandvicci),
                        executor.submit(send_sms.Sevilim),
                        executor.submit(send_sms.Seyhan),
                        executor.submit(send_sms.Sozcu),
                        executor.submit(send_sms.Sukruoglu),
                        executor.submit(send_sms.Tefal),
                        executor.submit(send_sms.Trendyol),
                        executor.submit(send_sms.Turuncu),
                        executor.submit(send_sms.Ulasim),
                        executor.submit(send_sms.Vakko),
                        executor.submit(send_sms.Voyage),
                        executor.submit(send_sms.Yemeksepeti),
                        executor.submit(send_sms.Yozgat),
                        executor.submit(send_sms.Zafron),
                    ]
                    wait(futures)
                    print(Fore.LIGHTRED_EX + "\nMenüye dönmek için 'enter' tuşuna basınız..")
                    input()
        except Exception as e:
            print(Fore.LIGHTRED_EX + f"Hata oluştu: {e}")
            sleep(3)
            continue